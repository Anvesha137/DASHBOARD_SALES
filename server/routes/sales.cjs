...
router.post('/', authenticateToken, requireRole('admin'), async (req, res) => {
    ...
    try {
        const { rows } = await db.query(
            `INSERT INTO sales_people (name, email) VALUES ($1, $2) ON CONFLICT (email) DO NOTHING RETURNING *`,
            [name, email]
        );
        if (rows.length === 0) {
            const existing = await db.query(`SELECT * FROM sales_people WHERE email = $1`, [email]);
